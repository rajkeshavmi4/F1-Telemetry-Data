...
async function loadTelemetry() {
  try {
    ...
    drawTrack(locA, locB, colorA, colorB);
    drawLineChart(el.speed, [
      { label: driverA.name_acronym, data: teleA.map(t => t.speed), color: colorA },
      { label: driverB.name_acronym, data: teleB.map(t => t.speed), color: colorB }
    ], { min: 0, suffix: " km/h" });
...
function drawPedalChart(teleA, teleB, driverA, driverB, colorA, colorB) {
  drawLineChart(el.pedal, [
    ...
    { label: `${driverA.name_acronym} brake`, data: teleA.map(t => t.brake), color: "#ffd166" },
    { label: `${driverB.name_acronym} brake`, data: teleB.map(t => t.brake), color: "#ffffff" }
  ], { min: 0, suffix: "%" });
}

function drawTrack(locA, locB, colorA, colorB) {
  const ctx = setupCanvas(el.track);
  const { width, height } = el.track;
  ...
  drawPath(ctx, locA.map(map), colorA, 5);
  drawPath(ctx, locB.map(map), colorB, 3);
  drawLegend(ctx, [
    { text: getDriver(el.driverA.value).name_acronym, color: colorA },
...
function setupCanvas(canvas) {
  ...
  canvas.height = Math.max(1, Math.floor((rect.width * Number(canvas.getAttribute("height")) / Number(canvas.getAttribute("width"))) * ratio));
  const ctx = canvas.getContext("2d");
  ctx.setTransform(ratio, 0, 0, ratio, 0, 0);
  canvas.width = rect.width;
  canvas.height = rect.width * Number(canvas.getAttribute("height")) / Number(canvas.getAttribute("width"));
